...
DemoApp.prototype.init = function() {
    ...
    self._registerName      = apphtml.find("#registerName");
    self._registerEmail     = apphtml.find("#registerEmail");
    self._registerPass      = apphtml.find("#registerPass");
    self._repeatPass        = apphtml.find("#repeatPass");
    self._registerInButton  = apphtml.find("#registerInButton");
    ...
    self._registerInButton.click(function() {
        if (self._registerName.val() == "") {
            self._showError("register name cannot be empty");
        }
        else if (self._registerEmail.val() == "") {
            self._showError("register email cannot be empty");
        }
        else if (self._registerPass.val() == "") {
            self._showError("register pass cannot be empty");
        }
        else if (self._registerPass.val() != self._repeatPass.val()) {
            self._showError("password not same");
        }
        else { 
            self.register();
        }
...
DemoApp.prototype.signIn = function() {
    ...
    var pass = self._pass.val();
    if (pass === "") {
        self._showError("PLEASE ENTER pass");
        return;
    }
    ...
    var values = {"username": username, "pass": pass};
...
DemoApp.prototype.register = function() {
    var self = this;

    var username = self._registerName.val();
    if (username === "") {
        self._showError("PLEASE ENTER username");
        return;
    }

    var pass = self._registerPass.val();
    if (pass === "") {
        self._showError("PLEASE ENTER pass");
        return;
    }

    var email = self._registerEmail.val();
    if (email === "") {
        self._showError("PLEASE ENTER email");
        return;
    }
    var serverAddr = document.location.host
    self._httpServerAddr = "http://" + serverAddr + "/" + HTTP_ENTRY + "/";
    self._websocketServerAddr = "ws://" + serverAddr + "/" + WEBSOCKET_ENTRY + "/";
    ...
    self._appendLogMark();
    self._appendLog("REGISTER " + serverAddr);

    var values = {"username": username, "pass": pass, "email": email};
    self._sendHttpRequest("user.register", values, function(res) {
        if (!self._validateResult(res, ["success"])) {
            self._state = State.IDLE;
            self._showError("Get invalid result");
            self._appendLog(res.toString());
        } else {
...
DemoApp.prototype._updateUI = function() {
    ...
    self._pass.prop("disabled", state != State.IDLE);
